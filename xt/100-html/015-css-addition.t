...
throws-like { $processor = Pod::To::HTML.new(:css-type<load>, :css-src('assets/xxpod.css') ) },
        X::ProcessedPod::HTML::InvalidCSS::BadSource,
        message=>/'assets/xxpod.css does not exist as a text file'/,
        "Catches bad file";
throws-like { $processor = Pod::To::HTML.new(:css-type<other>, :css-src<Something>) },
...
lives-ok { $processor = Pod::To::HTML.new(:css-type<link>, :css-src($link)) }, 'valid :css-type<link> and :css-src work';
...
$fn.IO.spurt: q:to/CSSEND/;
    ...
    CSSEND
lives-ok { $processor = Pod::To::HTML.new(:css-type<load>, :css-src($fn)) }, 'valid :css-type<load> and :css-src work';
$fn.IO.unlink if $fn.IO.e;
$rv = $processor.source-wrap;
like $rv, /
'<head' .+ '<style' .+
...
lives-ok { $processor = Pod::To::HTML.new(:favicon-src('favicon-new')) }, 'favicon file now exists';
